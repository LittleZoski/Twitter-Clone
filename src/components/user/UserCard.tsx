...
import { Card, Image, Text, Badge, Button, Group, Box, TextInput, Checkbox } from "@mantine/core";
...
function UserCard({ user }: { user: User }) {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) => (/^[0-9a-zA-Z]{2,12}$/.test(value) ? null : 'Invalid name'),
      about: (value) => (/^[\s\S]{3,120}$/.test(value) ? null : 'Invalid about'),
    },
  });
  ...
  return (
    <Card shadow="sm" p="lg" radius="md" mb="lg" withBorder style={{width:400}}>
      ...
      <Text size="sm" color="dimmed">
        ...
        <Group>
          user's real name: {currentuser.name}
        </Group>
        <Group>
          User Bio: {currentuser.about}
        </Group>
      </Text>
      ...
      <Button variant="light" color="blue" fullWidth mt="md" radius="md" onClick={()=>setOpen(!isOpen)}>
        Edit User profile
      </Button>
      {isOpen && currentuser.id===Number(localStorage.getItem("currentUserId")) && <Box sx={{ maxWidth: 300 }} mx="auto">
      <form onSubmit={form.onSubmit((values) => handlePatchUserInfor(values))}>
        <TextInput
          withAsterisk
          label="name"
